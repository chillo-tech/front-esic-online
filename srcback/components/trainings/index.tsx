...
import TrainingTable from '../items/TrainingTable';
...
function Trainings(params: Params) {
  ...
  const {
    ...
    queryKey: ["Trainings", (Object.values(state) as string[]).map((param: string) => slugify(String(param))).join('-')],
    queryFn: () =>
      read({
        fields: base,
        path: state?.trainingsParams?.path,
        limit: 10,
        filter: {
           "cpf": { "_eq": true },
