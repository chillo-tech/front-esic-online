...
function Search({ classes, isFocused }: Params) {
  ...
  useQuery<any>({
    queryKey: [
      'Trainings',
      (Object.values(query) as string[]).map((param: string) => slugify(String(param))).join('-'),
    ],
    queryFn: () =>
      fetchData({
        fields: 'id,libelle,slug',
        path: 'formations',
        filter: { libelle: { _icontains: query.trim() } },
        sort: 'libelle',
        limit: 10,
      }),
      onSuccess: (data: any) => {
  ...
  const handleSubmit = (event: React.SyntheticEvent) => {
    ...
    if(inputRef.current) {
      ...
      console.log('====================================');
      setData([]);
      setTrainingQuery(`${inputRef.current.value} `);
    }
  }
  return (
    <div
      className={classNames('search w-full', {
        'search-empty': !(data && data.length),
        'search-list': (data && data.length)
      })}
    >
