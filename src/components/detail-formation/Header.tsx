        ...
        <header className='bg-app-blue py-2 md:py-12'>
          <div className="md:px-0 container grid md:grid-cols-2">
            <div className="title">
              ...
              <div className="grid grid-cols-2 items-center gap-2 md:items-end md:w-4/5 justify-center">
                <AllTrainings 
                  training={training}
                  icon= {false}
                  link={`/nos-formations/votre-candidature?formation=${slugify(training.libelle)}-${training.id}`}
                  text="Je m'inscris"
                  classes='text-sm bg-white w-full text-app-blue font-light md:px-20 py-3 border hover:bg-transparent hover:text-white hover:border hover:border-white'
                />
                {
                   (training.programmepdf ) ? 
                      <button type="button" onClick={toogleDownloadForm}
                        className="text-sm text-white w-full md:mr-0 md:mt-0 md:px-4 justify-center items-center uppercase py-3 rounded-lg relative border border-app-white hover:bg-white hover:text-app-blue"
                      >
