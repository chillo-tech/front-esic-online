...
function ItemDetail({
  path,
  data,
  wrapper = "categories_id",
  subfield = "souscategories",
  subFieldWrapper = "sousCategories_id",
}: any) {
  return (
    <details className="mb-4 md:p-0">
      ...
      {data[wrapper][subfield]?.length ? (
        <ul className="grid gap-2 py-2">
          {data[wrapper][subfield].map((sub: any) => (
            <li key={sub[subFieldWrapper].id}>
              <Link
                href={`/${path}/${slugify(data[wrapper]?.libelle)}-${
                  data[wrapper]?.id
                }/${slugify(sub[subFieldWrapper].libelle)}-${
                  sub[subFieldWrapper].id
                }`}
