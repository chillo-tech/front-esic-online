...
function ItemDetail({path, data, wrapper='categories_id', subfield='souscategories', subFieldWrapper='sousCategories_id'}: any) {
  ...
  return (
    <details className='mb-4 md:p-0'>
      <summary className='focus:ring-0 focus:ring-offset-0 hover:shadow-none focus:shadow-none cursor-pointer text-lg md:text-3xl font-extrabold text-gray-500 flex items-center justify-between w-full py-1 text-left border-b  border-green-700 focus:ring-gray-200'>
        <span>{data[wrapper]?.libelle}</span>
        <span>
      ...
      {
         data[wrapper][subfield]?.length ? (
            <ul className='grid gap-2 py-2'>
              {
               data[wrapper][subfield]
                  .map((sub: any)=>(
                    <li key={sub[subFieldWrapper].id}>
                        <Link 
                          href={`/${path}/${slugify(data[wrapper]?.libelle)}-${data[wrapper]?.id}/${slugify(sub[subFieldWrapper].libelle)}-${sub[subFieldWrapper].id}`}
