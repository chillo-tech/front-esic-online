...
function Training({ id, slug }: { id: string; slug: string }) {
  ...
  useQuery<any>({
    enabled: !!(
      trainingId &&
      training?.souscategories.filter(
        (souscategory: any) =>
          souscategory != null && souscategory.souscategories_id != null
      )[0]?.souscategories_id?.id
    ),
    queryKey: [
      "SousCategories",
      training?.souscategories[0]?.souscategories_id?.id,
    ],
    queryFn: () =>
      getSubCategories({
        id: training?.souscategories.filter(
          (souscategory: any) =>
            souscategory != null && souscategory.souscategories_id != null
        )[0]?.souscategories_id?.id as string,
        trainingsLimit: 3,
      }),
    onSuccess: ({ data }: any) => {
    ...
    <OpenedLayout>
        ...
        <div className="md:relative">
                    ...
                    {training.sessions && training.sessions.length ? (
                      <div className="hidden md:block px-5 ">
                        <TrainingLocalisation
                          localisations={training.localisation}
                          classes="text-center md:text-center py-2"
                        />
                        <div className="sessions">
                          ...
                          {training?.sessions
                            .filter(
                              (session: any) => session.sessions_id != null
                            )
                            .filter(
                              (session: any) =>
                                new Date(session.sessions_id.debut).getTime() >=
                                new Date().getTime()
                            )
                            .slice(0, 4)
                            .sort(
                            ...
                            .map((item: any, index: number) =>
                              ...
                              Date.now() ? (
                                ...
                                >
                                  <p className="mb-0">
                                    Du{" "}
                                    {getDisplayedDate(item.sessions_id.debut)}
                                  </p>
                                  <p className="mb-0">
          ...
          <div className="container py-4 grid md:grid-cols-7">
            <div className="md:col-span-4">
              ...
              {training.articles
                .filter((article: any) => article.articles_id != null)
                .map((article: any) => (
                  <Card
      ...
      <div className="grid gap-4 md:py-8 md:grid-cols-3 md:px-0 container relative bg-white">
        {relatedTraining
          .slice(0, 3)
          .filter(
            ({ formations_id }: { formations_id: string }) =>
              formations_id != null
          )
          .map((training: any, index: number) => (
            <HomeTrainingItem
              ...
              displayInfos={false}
              link={`/nos-formations/${slugify(
                training.formations_id.libelle
              )}-${training.formations_id.id}`}
