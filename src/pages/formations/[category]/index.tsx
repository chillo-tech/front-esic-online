...
function Category({id, libelle, link}: {id:string, libelle: string, link: string}) {
  const  { isSuccess, isLoading, data: category } = useQuery<any>({
  ...
  return (
    <OpenedLayout>
      ...
      ): null }
      {
        isSuccess? (
          <>
            <PageHeader title={category.data.data.titre} description={category.data.data.description}/>
            <main className='container mx-auto pt-5 pb-10'>
            <section className="grid md:grid-cols-3 gap-6">
              {category.data.data.souscategories.map((souscategory: any, index: number) => (
                <HoverCard
                  ...
                  link={`/formations/${link}/${slugify(souscategory?.sousCategories_id.libelle)}-${id}`}
                />
...
export async function getServerSideProps(context: any) {
  ...
  const libelle = params.category.substring(0, params.category.lastIndexOf("-"));
