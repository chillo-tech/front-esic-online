...
export const getMonthLabel = (entry: Date) => {
  return months[entry.getMonth()];
};
...
export const getSmallDisplayDate = (entry: Date) => {
  return `${entry.getDate()} ${getSmallMonthLabel(entry)} ${entry.getFullYear()}`;
};
